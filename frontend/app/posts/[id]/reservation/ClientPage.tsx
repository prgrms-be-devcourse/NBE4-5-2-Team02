...
export default function ClientPage({postid}: { postid: number }) {
    ...
    const [, setReservedDates] = useState<Date[]>([]);
    ...
    // availabilities를 순회하며 필요한 데이터 추출
    const extractedTimes = availabilities.map((item) => {
        ...
        return {
            ...
            endDate,         // 종료 날짜
            endTime,         // 종료 시간
            recurrenceDays: item.recurrenceDays, // 반복 주기 (숫자)
            recurring: item.recurring,           // 반복 여부 (boolean)
            originalDate: item.date              // 원래 날짜 (nullable string)
        };
    });
    console.log("이용 가능 날짜 시간 및 추가 데이터", extractedTimes);
    ...
    const isClickableDate = (date: Date) => {
        const result = extractedTimes.some((time) => {
            ...
            // 반복 여부가 true인 경우, 현재 날짜가 반복 주기에 해당하는지 확인
            if (time.recurring && time.recurrenceDays > 0) {
                const originalDate = moment(time.startDate).startOf("day"); // 반복의 기준이 되는 시작 날짜
                const diffDays = moment(date).diff(originalDate, "days");

                // 로그 출력: 반복 주기 계산
                console.log("반복 주기:", time.recurrenceDays);
                console.log("현재 날짜와 원래 날짜의 차이:", diffDays);

                // 반복 주기에 해당하면 클릭 가능
                if (diffDays >= 0 && diffDays % time.recurrenceDays === 0) {
                    console.log("반복 주기에 해당하여 클릭 가능!");
                    return true;
                }
            }
        ...
        // 최종 결과 로그 출력
        console.log("최종적으로 클릭 가능한가?", result);
        return result;
    };
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-green-50 p-8">
                <div className="flex items-center gap-4 pb-6">
                    {/* 설명 섹션 - 확대된 배너 */}
                    <div
                        className="flex items-center gap-4 bg-gradient-to-r from-green-50 to-green-100 px-5 py-3 rounded-xl border-2 border-green-200 min-w-[240px] hover:shadow-md transition-shadow">
                        <svg
                        ...
                        <div className="flex flex-col gap-1">
                            <h3 className="text-base font-semibold text-gray-700 flex items-center gap-2">
                                <span>툴게더 예약 관리</span>
                                <span
                                    className="bg-green-600 text-white px-2.5 py-1 rounded-full text-sm tracking-tight">
                실시간 동기화
              </span>
                            </h3>
                ...
                <div
                    className="w-full max-w-6xl mx-4 bg-white rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 hover:shadow-3xl">
                    <Calendar
                        localizer={localizer}
                        startAccessor="start"
                        endAccessor="end"
                        ...
                        components={{
                            toolbar: (props) => (
                                <div
                                    className="flex items-center justify-between px-6 py-4 bg-white border-b border-green-100 shadow-sm">
            ...
            ${
                ...
                <div
                    className="flex flex-col items-center mt-10 w-full max-w-2xl mx-auto p-6 bg-green-50 rounded-xl border-2 border-green-200 shadow-sm">
                    {/* 안내 문구 섹션 */}
